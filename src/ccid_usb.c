...
int get_ccid_usb_device_path(int reader_index, unsigned char *buf, size_t *buflen)
{
    ...
    s = snprintf(buf, *buflen, "%u-%u", libusb_get_bus_number(dev), ports[0]);
	if (s < 0 || (len += s) >= *buflen) {
		res = IFD_ERROR_INSUFFICIENT_BUFFER;
		goto end;
	}
    ...
    for (int i = 1; i < nports; i++) {
        s = snprintf(buf + len, *buflen - len, ".%u", ports[i]);
		if (s < 0 || (len += s) >= *buflen) {
			res = IFD_ERROR_INSUFFICIENT_BUFFER;
			goto end;
		}
	...
	s = snprintf(buf + len, *buflen - len, ":%u.%u", cfg->bConfigurationValue,
			usbDevice[reader_index].interface);
	if (s < 0 || (len += s) >= *buflen) {
		res = IFD_ERROR_INSUFFICIENT_BUFFER;
		goto end;
